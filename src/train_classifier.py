...
# Binary classification training with 5-fold cross-validation
...
def train_one_fold(train_dataset, val_dataset, fold_idx, args, device):
    ...
    # Model uses DEFAULTS for architecture parameters
    ...
    for epoch in range(args.epochs):
        ...
        for g, virus, ecfp, unimol, y in train_loader:
            ...
            y = y.to(device)
            optimizer.zero_grad()
            logits = model(g, virus, ecfp, unimol)
    ...
    model_dir = os.path.join(args.output_dir, 'classifier')
    os.makedirs(model_dir, exist_ok=True)
    torch.save(best_state, os.path.join(model_dir, f'fold_{fold_idx}.pth'))
    return model, best_val_auc

def evaluate_classifier(model, dataset, device, batch_size):
...
def main():
    parser = argparse.ArgumentParser(description='Train DeepMolVir Classifier')
    ...
    virus_std, ecfp_std, unimol_std, graphs, y, valid_idx, scalers = load_and_prepare_data(
        args.smiles_file, args.virus_npz, args.unimol_npy, task='classification'
    )
    ...
    print("\n========== Starting 5-Fold Cross-Validation (Classification) ==========")
    fold_val_aucs = []
    fold_test_metrics = []
    ...
    for fold_idx, (train_idx_fold, val_idx_fold) in enumerate(cv_splits):
        ...
        train_ds = MultiModalDataset(
            ...
            all_labels[train_idx_fold]
        )
        val_ds = MultiModalDataset(
            all_virus[val_idx_fold], all_ecfp[val_idx_fold],
            all_unimol[val_idx_fold],
            [all_graphs[i] for i in val_idx_fold],
            all_labels[val_idx_fold]
        )
        model, best_val_auc = train_one_fold(train_ds, val_ds, fold_idx, args, device)
        fold_val_aucs.append(best_val_auc)

        test_metrics, _, _ = evaluate_classifier(model, test_dataset, device, args.batch_size)
        fold_test_metrics.append(test_metrics)
        print(f"    Val AUC: {best_val_auc:.4f} | Test AUC: {test_metrics['auc']:.4f}")
    ...
    print(f"Validation AUC (mean +/- std): {mean_val_auc:.4f} +/- {std_val_auc:.4f}")
    ...
    with open(os.path.join(args.output_dir, 'classifier_results.txt'), 'w') as f:
        f.write("DeepMolVir Classifier 5-Fold CV Results\n")
        f.write(f"Validation AUC (mean +/- std): {mean_val_auc:.4f} +/- {std_val_auc:.4f}\n")
        for k in mean_test:
