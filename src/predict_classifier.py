...
# Predict antiviral activity (binary classification) using the 5-fold ensemble.
...
DEFAULT_MODEL_DIR = os.path.join(PROJECT_ROOT, "models", "classifier")
DEFAULT_SCALER_DIR = os.path.join(PROJECT_ROOT, "models", "scalers")
DEFAULT_OUTPUT = os.path.join(DATA_DIR, "casestudy1_classifier_predictions.csv")
...
# --------------------------- Main ---------------------------
def main():
    parser = argparse.ArgumentParser(description='DeepMolVir Classifier Prediction')
    ...
    all_probs = np.array(all_probs)  # (5, n_samples)
    mean_prob = np.mean(all_probs, axis=0)
    std_prob = np.std(all_probs, axis=0)
    pred_label = (mean_prob >= 0.5).astype(int)

    # 8. Save results
    df['probability_active'] = mean_prob
    df['probability_std'] = std_prob
    df['predicted_label'] = pred_label
    df['prediction'] = df['predicted_label'].map({1: 'Active', 0: 'Inactive'})

    df.to_csv(args.output, index=False)
    print(f"Predictions saved to {args.output}")

    print("\nFirst 5 predictions:")
    print(df[['Smiles', 'vVirus name', 'probability_active', 'prediction']].head())
